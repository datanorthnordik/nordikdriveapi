...
func RegisterRoutes(r *gin.Engine, fileService FileServicePort, logService LogServicePort) {
	...
	userGroup.Use(middlewares.AuthMiddleware())
	{
		...
		userGroup.POST("/achiever-stories/request", fileController.CreateAchieverStoryRequest)
		...
		userGroup.GET("/achiever-stories/:rowId", fileController.GetAchieverStoriesByRow)
		userGroup.GET("/photo/:photoId", fileController.GetPhoto)
		userGroup.GET("/doc/:docId", fileController.GetDoc)
		userGroup.GET("/achiever-stories/download/:storyId", fileController.DownloadAchieverStory)
		userGroup.GET("/achiever-stories/request/download/:storyId", fileController.DownloadRequestedAchieverStory)
